...
void HariMain(void) {
	char *vram; //声明的是p，*p相当于汇编中BYTE[p]，*p不是变量，p是地址变量
	int xsize, ysize;
...
void set_palette(int start, int end, unsigned char *rgb) {
	int i, eflags;
	eflags = io_load_eflags();	//记录EFLAGS的值
	io_cli();	//中断许可标志设为0，将忽略中断请求
	...
	io_store_eflags(eflags);	//EFLAGS值恢复
	return;
}
